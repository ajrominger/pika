...
getdfun <- function(s) {
    switch(s$model, 
           'fish' = function(x, ...) pfish(x, s$MLE, ...),
           'plnorm' = function(x, ...) pplnorm(x, s$MLE[1], s$MLE[2], ...),
           'stick' = function(x, ...) pstick(x, s$MLE, ...),
           'tnegb' = function(x, ...) ptnegb(x, s$MLE[1], s$MLE[2], ...),
           'tpois' = function(x, ...) ptpois(x, s$MLE, ...))
}
